...
module.exports = (router) => {
	...
	router.get('/savetarget/:id/:savedate/:weight', async (ctx, next) => {
		const { id, savedate, weight } = ctx.params;
		let datas = JSON.parse(fs.readFileSync('targetDatas.json', 'utf8'));
		let newestData = datas[id];
		if (newestData) {
			newestData = {"date": savedate, "weight": weight};
		} else {
			datas[id] = {"date": savedate, "weight": weight};
		}
		fs.writeFile('targetDatas.json', JSON.stringify(datas), (err) => {
			if (err) throw err;
			ctx.body = {
				success: true,
				list: {"date": savedate, "weight": weight},
			};
			console.log('文件已被保存');
		  });
